...
namespace WebApiMovil.DataLayer
{
    public class AsignarDA
    {
        ...
        public Recurso RecursoCRUD(Recurso entidad)
        {
            try
            {
                using (SqlConnection conection = new SqlConnection(ConfigurationManager.ConnectionStrings["cnxLaptop"].ConnectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("SP_INSERT_PROGRAMADOR", conection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@id", entidad.idProgramador);
                        ...
                        using (SqlDataReader dr = command.ExecuteReader())
                        {
                            if (dr.HasRows)
                            {
                                while (dr.Read())
                                {
                                    if (!dr.IsDBNull(dr.GetOrdinal("idProgramador")))
                                        entidad.idProgramador = dr.GetInt32(dr.GetOrdinal("idProgramador"));
        ...
        public List<Empleado> ListarProgramador(Proyecto entidad)
        {
            ...
            try
            {
                using (SqlConnection conection = new SqlConnection(ConfigurationManager.ConnectionStrings["cnxLaptop"].ConnectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("[SP_OBTENER_PROGRAMADOR]", conection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        ...
                        using (SqlDataReader dr = command.ExecuteReader())
                        {
                            if (dr.HasRows)
                            {
                                ...
                                while (dr.Read())
                                {
                                    ...
                                    if (!dr.IsDBNull(dr.GetOrdinal("Apellidos")))
                                        empleado.Apellidos = dr.GetString(dr.GetOrdinal("Apellidos"));
                                    if (!dr.IsDBNull(dr.GetOrdinal("Nombres")))
                                        empleado.Nombres = dr.GetString(dr.GetOrdinal("Nombres"));
                                    if (!dr.IsDBNull(dr.GetOrdinal("cargo")))
                                        empleado.cargo = dr.GetString(dr.GetOrdinal("cargo"));
                                    Lista.Add(empleado);
                                }
                            }
                        }
